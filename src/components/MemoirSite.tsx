...
interface MemoirSiteProps {
  onBack: () => void;
  onOpenThankYou: () => void;
}

export default function MemoirSite({ onBack, onOpenThankYou }: MemoirSiteProps) {
  ...
  const scrollToTop = () => {
    const element = document.getElementById('memoir-top');
    if (element) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[60] bg-[#FFD1DC] overflow-y-auto font-serif selection:bg-pink-300"
    >
      <div id="memoir-top" />
      
      {/* Progress Bar */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-1 bg-pink-500 origin-left z-[70]"
        style={{ scaleX }}
      />
      ...
      <motion.button
        ...
        onClick={scrollToTop}
        className="fixed bottom-8 right-8 p-4 bg-white/80 backdrop-blur-md rounded-full shadow-lg border border-pink-100 text-pink-500 z-[70] hover:bg-white transition-all"
      >
